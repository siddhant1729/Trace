...
def _parse_json_objects(raw: str, required_keys: set) -> List[dict]:
    ...
    # Attempt 2 — walk character-by-character to extract balanced brace blocks
    i, n = 0, len(cleaned)
    while i < n:
        if cleaned[i] != "{":
            i += 1
...
def _validate_edges(edges: List[dict], valid_ids: set) -> List[dict]:
    """Removes edges with unknown IDs, self-loops, and duplicates."""
    seen = set()
    clean = []
    for ed in edges:
        f, t = ed["from"], ed["to"]
...
def _reindex_nodes(nodes: List[dict]) -> List[dict]:
    """Re-indexes nodes to sequential IDs n1, n2, n3… Returns (nodes, id_map)."""
    reindexed = []
    id_map = {}
...
def vision_parser_node(state: TraceState):
    ...
    Detects diagram type, extracts nodes/edges, resizes image, validates IDs.
    """
...
def rag_node(state: TraceState):
    """Answers the user query using detected entities, edges, and conversation history."""
...
def analysis_node(state: TraceState) -> dict:
    """Runs rag_node and code_gen_node concurrently via ThreadPoolExecutor."""
