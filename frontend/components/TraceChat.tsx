...
import { Upload, Send, Terminal, X, ImageIcon, Code2, GitBranch } from "lucide-react";
...
export default function TraceChat() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult(data);
      // Auto-switch to connections tab if edges were found
      if (data.edges?.length > 0) setActiveTab("connections");
    } catch (err: unknown) {
  ...
  // Which labels are involved in the hovered edge (for highlight)
          ...
          {result && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
              ...
              <div className="bg-black/40">
                ...
                {/* Reply */}
                <div className="p-5 pb-3 font-mono text-sm space-y-2">
                ...
                {/* Tab bar */}
                <div className="flex gap-1 px-5 border-b border-white/[0.05]">
                  {tabs.map((tab) => (
                          ...
                          {result.entities.map((e, i) => {
                            ...
                            return (
                              <motion.span key={i}
                                ...
                                title={e.bbox?.length === 4 ? `bbox: [${e.bbox.join(", ")}]` : undefined}
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}
                      ...
                      {(!result.edges || result.edges.length === 0) ? (
                        <p className="text-xs text-slate-600 font-mono">
                          No directed edges detected in this diagram.
                        </p>
                      ) : (
                        result.edges.map((ed, i) => {
                          ...
                          return (
                            <motion.div key={i}
                              ...
                              onMouseEnter={() => { setHoveredEdge(i); setActiveTab("entities"); setTimeout(() => setActiveTab("connections"), 10); }}
